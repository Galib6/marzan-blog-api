...
export class InternalBannerController {
  ...
  @Get()
  async findAll(@Query() query: FilterBannerDTO): Promise<SuccessResponse | Banner[]> {
    return this.service.findAllBase(query, { relations: this.RELATIONS });
  }

  @Get(':id')
  async findById(@Param('id') id: string): Promise<Banner> {
  ...
  @Patch(':id')
  async updateOne(@Param('id') id: string, @Body() body: UpdateBannerDTO): Promise<Banner> {
